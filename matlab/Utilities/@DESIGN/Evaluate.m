function [NewAlgs,Solution] = Evaluate(NewAlgs,Problem,Data,Setting,seedInstance)
% Evaluate the designed algorithm's performance. Save all solutions
% generated during algorithm execution, for calculating landscape features.
...
Solution = cell(numel(seedInstance),1);
for i = 1:length(NewAlgs)
    ...
    for j = 1:length(Problem(seedInstance))
        this_ins_Solution = SOLVE; % for saving all solutions that the algorithm generated in the instance
        switch Problem(seedInstance(j)).type{2}
            case 'static'
                for k = 1:Setting.AlgRuns
                    [ArchSolution,AllSolution,t] = RunDesign(Operator,Parameter,Problem(seedInstance(j)),Data(seedInstance(j)),Setting);
                    % save result for draw convergence curve in python
                    if Setting.AlgRuns == 1
                        %save(['ArchSolution',int2str(Setting.Problem_id)],'ArchSolution');
                    end
                    ...
                    this_ins_Solution = [this_ins_Solution,AllSolution];
                end
        ...
        this_ins_Solution = this_ins_Solution(2:end); % delete the first empty solution
        Solution{j} = [Solution{j},this_ins_Solution];
    end
    NewAlgs(i).performance = Performance;
end
end

function [ArchSolution,AllSolution,t] = RunDesign(Operator,Para,Problem,Data,Setting)
...
AllSolution  = Solution; % all solutions found during algorithm execution
...
elseif Setting.AlgP > 1 % if the designed algorithm has multiple search pathways
    ...
    while G <= Problem.Gmax && t < Tmax && min(ArchSolution.fits) > Thres
        ...
        for i = 1:Setting.AlgP
            ...
            allNew = [allNew,New];
            AllSolution = [AllSolution,allNew];
        end
